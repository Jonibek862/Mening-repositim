...
def start(message):
    bot.send_message(message.chat.id, 'Salom! Shahar nomini kiriting, men sizga 5 kunlik ob-havo prognozini yuboraman.')
...
def send(message):
    ...
    if data.get('cod') != '200':
        bot.send_message(message.chat.id, 'Shahar topilmadi. Iltimos, to\'g\'ri nom kiriting.')
    else:
        forecast = f"5 kunlik ob-havo - {shahar}:\n\n"
